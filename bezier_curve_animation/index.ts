...
function mouseDown(e: MouseEvent) {
  ...
  points.forEach((p) => {
    const mouseOnPoint =
      Math.sqrt((p.x - e.clientX) ** 2 + (p.y - e.clientY) ** 2) < r;
    if (mouseOnPoint) {
      draggablePoint = p;
...
function bezier(p0: P, p1: P, cp0: P, cp1: P) {
  let from = p0;
